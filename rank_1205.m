...
% load all humming record sound and rank
all_song = dir("../testSongs/*.wav");
hit_rate = 0;
k = length(all_song);
outRank={};
outRankScore = [];
for s = 1:length(all_song)
    song1 = all_song(s).name;
    title = strsplit(string(song1),"_");
    title = strsplit(title(2),".");
    title = title(1);
    song = "D:\NTUEE\4_grade\first_semester\topic\djj\TestSongs\"+song1;
    [h_midi,h_beats] = getMidiNBeatByHumming(song);
    distOfEachSong = zeros(1,length(dataset));
    for i = 1:length(dataset)
        distOfEachSong(i) = getSongMatching(dataset{i}.melody,h_midi,dataset{i}.beats,h_beats);
    end
    [score,index] = min(distOfEachSong);
    ori_index = find(titles == title);
    if length(ori_index) == 0
        disp(title);
        k = k-1;
        continue
    end
    ori_score = distOfEachSong(ori_index);
    sortDist = sort(distOfEachSong);
    rank = find(sortDist == ori_score,1);
    if rank >5
        outRank{end+1} = string(song1);
        outRankScore(end+1) = rank;
    end
    hit_rate = hit_rate + 1/rank;
end
% hit rate
hit_rate = hit_rate/k;
disp(hit_rate);
...
function [midi,beats] = getMidiNBeatByHumming(song)
    ...
    [midi,onset_set] = getMidi(x,fs,onset_set,25,shift);
...
function [midi,onset_set] = getMidi(x,fs,onset_set,smoother_width,shift)
    ...
    while i <length(onset_set)
        ...
        max_freq_magnitude = max(P1(low_threshold_of_segment:end));
        [a,loc] = findpeaks(P1(low_threshold_of_segment:end),"MinPeakHeight",max_freq_magnitude*0.2,"SortStr","descend","NPeaks",3);
        if length(loc) == 0
            onset_set(i+1) = [];
            base_freq(i) = [];
            continue;
        end
        b_f = min(loc) + low_threshold_of_segment;
        base_freq(i) = b_f*fs/seg_len;
        i = i+ 1;
...
function envA = getSignalEnvelope(x,amp_max,tau,shift,slot)
    ...
    envA = envelope/(0.1+0.3*mean_of_Ak);
end
